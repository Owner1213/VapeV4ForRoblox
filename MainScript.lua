...
shared.deffont = shared.deffont or nil
...
local function TextGUIUpdate()
	local suc, res = pcall(function()
		-- Ensure 'vapeInjected' and 'GuiLibrary.MainGui.ScaledGui' are not nil
		local scaledgui = vapeInjected and GuiLibrary.MainGui and GuiLibrary.MainGui.ScaledGui
		if scaledgui and scaledgui.Visible then
			local formattedText = ""
			local moduleList = {}

			-- Loop through objects and ensure v.Api exists and is valid
			for i, v in pairs(GuiLibrary.ObjectsThatCanBeSaved) do
				if v.Type == "OptionsButton" and v.Api and v.Api.Enabled then
					...
					if not blacklistedCheck then
						local extraText = v.Api.GetExtraText and v.Api.GetExtraText() or ""
						table.insert(moduleList, {Text = v.Api.Name, ExtraText = extraText ~= "" and " "..extraText or ""})
					end
				end
			end

			-- Ensure TextGUISortMode exists and has a valid value
			if TextGUISortMode and TextGUISortMode.Value == "Alphabetical" then
				table.sort(moduleList, function(a, b) return a.Text:lower() < b.Text:lower() end)
			else
				table.sort(moduleList, function(a, b)
					-- Ensure VapeText properties are valid
					if VapeText and VapeText.TextSize and VapeText.Font then
						return textService:GetTextSize(a.Text..a.ExtraText, VapeText.TextSize, VapeText.Font, Vector2.new(1000000, 1000000)).X > 
							   textService:GetTextSize(b.Text..b.ExtraText, VapeText.TextSize, VapeText.Font, Vector2.new(1000000, 1000000)).X
					else
						return false
					end
				end)
			...
			-- Ensure VapeText exists and has valid properties
			if VapeText then
			...
			-- Ensure the offsets array exists and is used properly
			local offsets = { 5, 1, 23, 23 }
			if TextGUI.GetCustomChildren().Parent then
				local parent = TextGUI.GetCustomChildren().Parent
				if (parent.Position.X.Offset + parent.Size.X.Offset / 2) >= (gameCamera.ViewportSize.X / 2) then
					-- Handle Right Alignment
					if VapeText and VapeTextExtra and VapeLogo then
			...
			-- Drawing logic for TextGUIMode "Drawing"
			if TextGUIMode and TextGUIMode.Value == "Drawing" then
				-- Remove previous labels and shadows
			...
			-- Clear old background children before creating new ones
			if VapeBackground then
				for i,v in pairs(VapeBackground:GetChildren()) do
					table.clear(VapeBackgroundTable)
					if v:IsA("Frame") then v:Destroy() end
				end

				-- Create background frames for each entry
				for i,v in pairs(backgroundList) do
	...
	-- Display warning if an error occurs
