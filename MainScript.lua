...
--shared.deffont = Enum.Font.Arial
		...
		if scaledgui and scaledgui.Visible then
			...
			if TextGUI.GetCustomChildren().Parent then
				...
				if (parent.Position.X.Offset + parent.Size.X.Offset / 2) >= (gameCamera.ViewportSize.X / 2) then
					...
					-- Ensure VapeText and related objects are valid before modifying properties
			...
			if TextGUIMode.Value == "Drawing" then
				-- Add similar nil checks here
			end

			-- Ensure VapeBackground and VapeBackgroundList exist before calling GetChildren
			if VapeBackground then
				for i, v in pairs(VapeBackground:GetChildren()) do
					table.clear(VapeBackgroundTable)
					if v:IsA("Frame") then v:Destroy() end
				end
				-- Add background frames logic...
			end

			-- Update UI logic with checks
